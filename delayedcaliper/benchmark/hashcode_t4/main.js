...
function main() {
    ...
    let logger = Util.getLogger('benchmark/hashcode_t4/main.js');
    let absConfigFile;
    if(typeof configFile === 'undefined') {
        absConfigFile = path.join(__dirname, 'configs/transaction_flow/medium.yaml');
    }
    else {
    ...
    if(typeof networkFile === 'undefined') {
        try{
            absNetworkFile = path.join(absCaliperDir, 'benchmark/hashcode_t4/fabric.json');
        }
        catch(err) {
