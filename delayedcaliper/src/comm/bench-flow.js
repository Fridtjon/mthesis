...
async function defaultTest(args, clientArgs, final) {
    ...
    let testIdx = 0;
    let sleepTime = 30000; // In millis. 1000 ms = 1 s. 30000 = 30 s
    for (let test of tests) {
        ...
        try {
            // Modified sleep code
            logger.info(`Waiting ${sleepTime / 1000} seconds before the transaction...`);
            await Util.sleep(sleepTime)
            logger.info(`Doing a new Transaction.`)
            logger.info(`Current time (STARTING): ${(new Date()).toJSON().slice(0, 19).replace(/[-T]/g, ':')}`)
            ...
            logger.info(`Current time (PASSED): ${(new Date()).toJSON().slice(0, 19).replace(/[-T]/g, ':')}`)
            // prepare for the next round
            if(!final || testIdx !== tests.length) {
                logger.info(`Waiting ${sleepTime / 1000} seconds for the next round...`);
                await Util.sleep(sleepTime);
...
module.exports.run = async function(configFile, networkFile) {
    ...
    try {
        ...
        let output = path.join(process.cwd(), `report-from-tests.html`);
